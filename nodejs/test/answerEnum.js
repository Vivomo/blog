...
for (let i = 0; i < max; i++) {
    let temp = i;
    let answers = new Array(10).fill(null).map(() => {
        let answer = temp % 4;
        temp >>= 2;
        return answer;
    });
    ...
    // 3. 3, 6, 2, 4 不同的一个
    let a3 = [2, 5, 1, 3];
    if(!a3.some((item, index) => {
        return answers[2] === index &&
            a3.filter((item, _index) => _index !== index).every(answerIndex => answers[answerIndex] !== answers[item]);
    })) {
        continue;
    }
    // 4. [1,5] [2,7] [1,9] [6,10] 两题相同的是
    if( ![[0, 4], [1, 6], [0, 8], [5, 9]].some((item, index) => {
        return answers[item[0]] === answers[item[1]] && answers[3] === index
    })) {
        continue;
    }
    // 5. 8 4 9 7, 那个和5相同
    ...
    if (![[1, 3], [0, 4], [2, 9], [4, 8]].some((item, index) => {
        return answers[item[0]] === answers[item[1]] && answers[item[1]] === answers[7] && answers[5] === index
        })) {
        continue;
    }
    // 7. c b a d 哪个最少?
    ...
    if (![5, 9, 1, 8].some((ansIndex, index) => {
        return (answers[0] === answers[5]) !== (answers[ansIndex] === answers[4]) && answers[8] === index;
        })) {
        continue;
    }
