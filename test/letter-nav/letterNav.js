class LetterNav{
    ...
    refreshTouchView(screenY) {
        let activeIndex = this.navPositionCache.findIndex(y => Math.abs(screenY - y) <= this.activeDistance);
        ...
        for (let i = startIndex; i <= endIndex; i++) {
            this.updateNavItemStyle(i, screenY);
        }
    }
     ...
     * @param screenY
     */
    updateNavItemStyle(index, screenY) {
        let nav = this.navItems[index];
        let percent = Math.abs(screenY - this.navPositionCache[index]) / (this.activeDistance * 2 * this.extent);
